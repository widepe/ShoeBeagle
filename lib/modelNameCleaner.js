...
function stripShoeTypeWords(text) {
  const s = normalizeWhitespace(String(text || ""));

  const m = s.match(
    /\b(trail running shoes?|road running shoes?|running shoes?|running shoe|trail shoes?|track(?: & field)? shoes?|race shoes?|walking shoes?|slip-?ons?|spikes?)\b/i
  );
  ...
  return {
    text: normalizeWhitespace(s.slice(0, m.index)),
    removed: true,
  };
}
...
function cleanModelName(input, opts = {}) {
  ...
  if (shoeTypeResult.removed) {
    s = normalizeWhitespace(shoeTypeResult.text);
    return {
      cleanedModel: s,
      modelBase: s,
      version: null,
  ...
  // If version exists, safest rule: keep everything up to version
  if (version) {
    s = keepUpToVersion(s, versionInfo);
  } else {
