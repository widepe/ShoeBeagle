...
function stripTrailingWidthCode(text) {
  // Remove standalone shoe width codes that appear as the last token in the string.
  // Codes: 2E, 4E, 6E, EE, EEEE, B, D, N, W, XW (case-insensitive)
  return normalizeWhitespace(
    String(text || "").replace(/\s+(?:EEEE|EE|[246]E|XW|[BDNW])\s*$/i, "")
  );
}
...
function cleanModelName(input, opts = {}) {
  ...
  // Strip trailing width codes (e.g. "Glycerin 22 2E" → "Glycerin 22")
  s = stripTrailingWidthCode(s);
